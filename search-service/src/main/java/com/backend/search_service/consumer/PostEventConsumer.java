...
import com.backend.search_service.event.PostIndexedResultEvent;
...
public class PostEventConsumer {
    ...
    public void onCreated(@Payload String payload,
        ...
        try {
            ...
            // Extract postId from payload for error result
            try {
                PostCreatedEvent event = objectMapper.readValue(payload, PostCreatedEvent.class);
                publishResult(event.getPostId(), false, e.getMessage());
            } catch (Exception ex) {
                log.error("Cannot publish failure result for post.created", ex);
            }
        }
    }
    ...
    private void publishResult(String postId, boolean success, String error) {
        try {
            PostIndexedResultEvent result = PostIndexedResultEvent.builder()
                    .postId(postId)
                    .success(success)
                    .error(error)
                    .build();
            String payload = objectMapper.writeValueAsString(result);
            kafkaTemplate.send("post.indexed.result", postId, payload);
            log.info("Published post.indexed.result | postId={} | success={}", postId, success);
        } catch (Exception e) {
            log.error("Failed to publish post.indexed.result for postId={}", postId, e);
        }
    }
    ...
    public void onUpdated(@Payload String payload,
        ...
        try {
            ...
            searchCacheService.invalidateSearchCache(); // ← Invalidate search cache
            idempotencyService.markAsProcessed("search-service", messageId);
