...
import com.backend.scan_service.services.ScanCacheService;
...
public class ScanController {
    ...
    public ResponseEntity<ApiResponse<ScanResponse>> save(
        ...
        return ResponseEntity.ok(ApiResponse.ok(
                scanCacheService.saveWithCacheInvalidation(getUserId(request), body)));
    }
    ...
    public ResponseEntity<ApiResponse<List<ScanResponse>>> getHistory(
            HttpServletRequest request) {
        List<ScanResponse> scans = scanCacheService.getHistoryWithCache(getUserId(request));
        log.debug("GET /api/v1/scans - retrieved {} scan records", scans.size());
        return ResponseEntity.ok(ApiResponse.ok(scans));
    }

    @Idempotent(ttlSeconds = 300) // Cache 5 minutes
    ...
    public ResponseEntity<ApiResponse<ScanResponse>> updateConv(
            ...
            HttpServletRequest request) {
        return ResponseEntity.ok(ApiResponse.ok(
                scanCacheService.updateConvWithCacheInvalidation(getUserId(request), id, body)));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> delete(
            @PathVariable UUID id,
            HttpServletRequest request) {
        scanCacheService.deleteWithCacheInvalidation(getUserId(request), id);
