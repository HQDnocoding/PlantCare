...
public class IdempotencyAspect {
    ...
    public Object handleIdempotentRequest(ProceedingJoinPoint joinPoint, Idempotent idempotent) throws Throwable {
        try {
            ...
            // Skip idempotency for multipart requests (file uploads cannot have body
            // hashed)
            String contentType = request.getContentType();
            if (contentType != null && contentType.startsWith("multipart/")) {
                log.debug("Multipart request detected, skipping idempotency");
                return joinPoint.proceed();
            }

            // Extract headers - MUST be present for idempotent operations
            String userId = request.getHeader("X-User-Id");
            String idempotencyKey = request.getHeader("X-Idempotency-Key");

            if (userId == null || idempotencyKey == null) {
                throw new IllegalStateException(
                        "Idempotent operation requires X-User-Id and X-Idempotency-Key headers");
            }
