...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
    ...
    if (!res.ok) {
        // Extract friendly error message from API response
        const errorCode = json.errorCode || `HTTP_${res.status}`;
        const friendlyMessage = ERROR_MESSAGES[errorCode] || json.message || `HTTP ${res.status}`;
        throw new ApiError(res.status, errorCode, friendlyMessage);
    }
