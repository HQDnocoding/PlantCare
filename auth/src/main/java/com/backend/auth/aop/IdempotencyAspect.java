...
public class IdempotencyAspect {
    ...
    public Object handleIdempotency(ProceedingJoinPoint joinPoint, Idempotent idempotent) throws Throwable {
        ...
        try {
            ...
            // Note: For unauthenticated requests, scopingId will be derived from
            // idempotency key hash (see below)
            // This prevents collision between different unauthenticated users

            // Check if this request was already processed (scoped by method + path)
            String method = request.getMethod();
            String path = request.getRequestURI();

            // For unauthenticated requests, use hash of idempotency key as namespace to
            // avoid collision
            UUID scopingId = userId;
            if (userId == null) {
                scopingId = UUID.nameUUIDFromBytes(idempotencyKey.getBytes(java.nio.charset.StandardCharsets.UTF_8));
                log.debug("Unauthenticated request - using key hash as scoping ID: {}", scopingId);
            }

            Optional<IdempotencyRecord> existingRecord = idempotencyService.getExistingRecord(idempotencyKey, scopingId,
                    method, path);
