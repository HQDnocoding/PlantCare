...
public class PostService {
    ...
    @Transactional
    public PostResponse updatePostByAdmin(UUID postId, String content, List<String> tags) {
        if (content == null || content.trim().isBlank()) {
            throw new AppException(ErrorCode.POST_CONTENT_REQUIRED);
        }
        Post post = postRepo.findById(postId)
        ...
        log.info("[Admin] Updated post {}", postId);
        return PostResponse.from(post, author, null);
    }
    ...
    @Transactional
    public void deletePostSaga(UUID postId) {
        // Compensation: delete post due to saga failure (no permission check needed)
        Post post = postRepo.findById(postId)
                .orElseThrow(() -> new AppException(ErrorCode.POST_NOT_FOUND));

        if (post.getDeletedAt() != null) {
            log.info("Post {} already deleted, skipping compensation deletion", postId);
            return;
        }
        ...
        if (post.getImagePaths() != null) {
            for (String path : post.getImagePaths()) {
                try {
                    ...
                    log.error("Failed to delete image during saga compensation | path={} | error={}",
                            path, e.getMessage());
                }
            }
        }
        ...
        post.softDelete();
        postRepo.save(post);
        log.info("Post deleted due to saga compensation | postId={}", postId);
    }
