...
async def get_conversations(request: Request):
    ...
    logger.info(f"GET /api/v1/history - user={user_id}")
    try:
        result = await asyncio.to_thread(storage_service.get_conversations, user_id)
        logger.info(f"Fetched {len(result)} conversations for user={user_id}")
        return result
    except Exception as e:
        logger.error(
            f"Get conversations failed for user={user_id}: {type(e).__name__}: {e}",
            exc_info=True,
        )
        raise HTTPException(
...
async def get_conversation_history(conv_id: str, request: Request):
    user_id = _get_user_id(request)
    logger.info(f"GET /api/v1/history/{conv_id} - user={user_id}")
    try:
        ...
        logger.info(
            f"Fetched {len(messages)} messages for user={user_id} conv={conv_id}"
        )
        return HistoryResponse(conv_id=conv_id, summary=summary, messages=messages)
    except Exception as e:
        logger.error(
            f"Get history failed for user={user_id} conv={conv_id}: {type(e).__name__}: {e}",
            exc_info=True,
        )
...
async def delete_conversation(conv_id: str, request: Request):
    user_id = _get_user_id(request)
    logger.info(f"DELETE /api/v1/history/{conv_id} - user={user_id}")
    try:
        messages_ref = (
        ...
        logger.error(
            f"Delete failed for user={user_id} conv={conv_id}: {type(e).__name__}: {e}",
            exc_info=True,
        )
        raise HTTPException(
