...
semaphore = asyncio.Semaphore(
    20
)  # Per-process concurrency limit; use Redis for distributed deployments
...
async def chat(
    ...
    if not message or not message.strip():
        logger.warning(f"[{x_correlation_id}] Invalid input - message is empty")
        raise HTTPException(status_code=400, detail="Message cannot be empty")

    if len(message) > 5000:
        logger.warning(
            f"[{x_correlation_id}] Invalid input - message too long: {len(message)} chars"
        )
        raise HTTPException(
            status_code=400, detail="Message exceeds 5000 character limit"
        )
